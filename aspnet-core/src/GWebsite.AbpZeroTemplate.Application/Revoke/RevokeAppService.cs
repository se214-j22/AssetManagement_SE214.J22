...
namespace GWebsite.AbpZeroTemplate.Web.Core.Revokes
{
    ...
    public class RevokeAppService : GWebsiteAppServiceBase, IRevokeAppService
    {
        ...
        public List<RevokeDto> GetListAssetNotApprove()
        {
            IQueryable<Revoke> query = revokeRepository.GetAll().Where(x => !x.IsDelete)
                .Where(x => x.StatusApproved == false);
            IQueryable<RevokeDto> assetDtoQuery = query.ProjectTo<RevokeDto>(query);
