...
namespace GWebsite.AbpZeroTemplate.Web.Core.Suppliers
{
    public class SupplierAppService : GWebsiteAppServiceBase, ISupplierAppService
    {
        ...
        public SupplierAppService(IRepository<Supplier, int> supplierRepository, IRepository<SupplierType, int> supplierTypeRepository)
        {
        ...
        {
            var entity = await this.supplierRepository.GetAllIncluding(p => p.Products).FirstOrDefaultAsync(x => x.Id == input.Id);
        ...
        {
            var query = this.supplierRepository.GetAllIncluding().Include(p => p.SupplierType).Include(p => p.Products);
            ...
            int status)
        {
            ...
            if (status == 3)
            {
                supTypes = await supplierTypeRepository.GetAllIncluding(p => p.Suppliers)
