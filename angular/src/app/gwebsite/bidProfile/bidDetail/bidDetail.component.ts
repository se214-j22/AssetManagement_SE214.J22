...
export class BidDetailComponent extends AppComponentBase implements AfterViewInit, OnInit {
     ...
     */
    filterText: string;
    //permission cho duyệt, thêm, xóa, sửa: Admin và Department tạo bidDetail đó.
    // duyệt: chỉ mỗi Admin đc duyệt
    // sửa, đóng: department tạo ra bidDetail đó và Admin.
    ...
    public bidDetailCodeFilter = '';
    public bidDetailNameFilter = '';

    // -những dự án của năm cũ, sẽ tự động close (mỗi lần đến 1/1/newyear, sẽ trigger cho nó close hết bidDetails năm cũ),
    //      dù có đc approved hay chưa.
    // -những dự án của năm hiện tại: chỉ dc phép close khi nó chưa đc approved.
    public bidDetailFakes = [
        {
            id: 1,
            code: 'S001',
            ...
            unitPrice: '20000',
            calUnit: 'VND',
            description: 'This is bidDetail item',
    ...
    /**
     * Hàm get danh sách BidDetail
     * @param event
     */
    getBidDetails(event?: LazyLoadEvent) {
        ...
        this.primengTableHelper.records = this.bidDetailFakes;
    ...
    applyFilters(): void {
        ...
        this._router.navigate(['app/gwebsite/bidDetail', {
    ...
    //hàm show view create BidDetail
    createBidDetail() {
    }

    public searchBidDetail(): void {
        //3 params filter FE truyền vào api
        // filter, values default = ''
        console.log(this.status + '--' + this.bidDetailCodeFilter + '--' + this.bidDetailNameFilter);
    }
    ...
    public actionPCItem(id: number, row: any): void {
        if (this.isPermissionEditCloseActive) {
            // dựa vào id, set status cho bidDetail là close nếu nó đang open và ngược lại.
